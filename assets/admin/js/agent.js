$(document).ready(function () {
    $("#agent-form").validate({
        ...
        submitHandler: function (form) {
            var formData = new FormData($('#agent-form')[0]);
...
$(document).on('click', '.agentview', function () {
    var empid = $(this).attr('data-id');
    if (empid)
    {
        $.ajax({
            url: baseurl + 'agent/view',
            type: 'POST',
            data: {empid: empid},
            dataType: 'json',
            async: false,
            ...
            success: function (response) {
                if (response.status)
                {
                    ...
                    $("#agentviewModal").css({"display": "block"});
                } else
                {
                    openDanger(response.msg);
                }
...
$(document).on("click", ".close, .closebtn", function () {
    $("#agentviewModal").css({"display": "none"});
...
$(document).on('click', '.switchstatus', function () {
    ...
    {
        $.ajax({
            url: baseurl + 'agent/changestatus',
            type: 'POST',
...
$(document).on('click', '.agentdelete', function () {
    var empid = $(this).attr('data-id');
    if (empid)
    {
        $.ajax({
            url: baseurl + 'agent/deletepopup',
            type: 'POST',
            data: {empid: empid},
            dataType: 'json',
            async: false,
            ...
            success: function (response) {
                ...
                {
                    $("#popupshow").html(response.viewhtml);
                    $("#deleteagentmodal").css({"display": "block"});
                } else
                {
                    openDanger(response.msg);
                }
...
$(document).on('click', '.downloadexport', function () {
    $.ajax({
        url: baseurl + 'agent/downloadexcel',
        type: 'POST',
        dataType: 'json',
        async: false,
        beforeSend: function () {
