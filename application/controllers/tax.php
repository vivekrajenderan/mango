...
class Tax extends CI_Controller {
    ...
    public function index() {
        $params['select'] = array("id", "status", "taxname", "percentage");
        $data['list'] = $this->dbmodel->getGridAll('tax', $params);
        $this->load->view('includes/header');
        $this->load->view('tax/list', $data);
        $this->load->view('includes/footer', array('jsfile' => array_merge($this->config->item('jsfile')['datatable'], $this->config->item('jsfile')['validation'], $this->config->item('jsfile')['tax'])));
    }
    ...
    public function add($id = NULL) {
        ...
        if (!empty($id)) {
            ...
            if (count($data_list) > 0) {
                ...
                $rsttaxcondition_array['md5(fk_tax_id)'] = $_POST['ecodeid'];
                $taxgroup_list = $this->dbmodel->getAll('taxgroup', $rsttaxcondition_array);
                $data['grouprsttax'] = (count($taxgroup_list) > 0) ? $taxgroup_list : array();
            } else {
